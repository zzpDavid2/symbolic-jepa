...
# %% [markdown]
# # JEPA pretraining — v2 corrected baseline, **8-layer decoder**
#
...
#
# **The only difference from `jepa_pretrain_v2.ipynb` is `N_LAYERS = 8` instead
# of `4`**, to match the SymbolicGPT baseline's decoder depth
# (`symbolicgpt_baseline.py`: `N_LAYER = 8`, `N_HEAD = 8`). Everything else —
# data, splits, seeds, JEPA objective, schedule, evaluation — is unchanged, so
# the two notebooks form a depth ablation. `EXPERIMENT_VERSION` carries an `_l8`
# suffix, so checkpoints, logs and `metrics.json` land under a separate root and
# no 4-layer artifact is read, resumed from, or overwritten (the shapes would
# not load anyway).
#
# ```text
...
#
# _l8: decoder depth 4 -> 8 to match SymbolicGPT. Every transformer weight
# tensor changes count, so no 4-layer checkpoint can load here; the suffix keeps
# the two depths in disjoint checkpoint/log/metrics roots and makes a mix-up
# structurally impossible rather than merely unlikely.
EXPERIMENT_VERSION = 'jepa_pretrain_v2_vocab40_matched_l8'
...
# ── Model / optimisation (unchanged from v1 except N_LAYERS) ───────────
...
D_MODEL = 512
N_HEADS = 8
N_LAYERS = 8                  # 4 in jepa_pretrain_v2; 8 matches SymbolicGPT
DROPOUT = 0.2
